...
static _lock_t s_connected_lock;
static bool s_connected = 0;
static TimerHandle_t xConnHandle;

/********************************
 * GLOBAL VARIABLE DEFINITIONS
 *******************************/

float g_volume_percent = 0.5f;
int8_t g_volume_db = 0;

/********************************
 * STATIC FUNCTION DEFINITIONS
 *******************************/

static void xConnect_cb(TimerHandle_t xTimer){
	ESP_LOGI("DEBUG", "changing internal state");
...
static void bt_av_hdl_avrc_ct_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_AVRC_CT_METADATA_RSP_EVT: {
        ESP_LOGI(BT_RC_CT_TAG, "AVRC metadata rsp: attribute id 0x%x, %s, strlen %d", rc->meta_rsp.attr_id, rc->meta_rsp.attr_text, strlen((const char *)rc->meta_rsp.attr_text));
